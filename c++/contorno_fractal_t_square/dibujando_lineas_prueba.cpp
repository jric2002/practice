...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT mensaje, WPARAM wParam, LPARAM lParam) {
  switch (mensaje) { // Manipulador de mensaje
    ...
    case WM_PAINT:
      ...
      static TCHAR titulo3[] = "Prueba 3:";
      static TCHAR prueba_31[] = "* 1 linea se dibuja de derecha a izquierda (<-).";
      static TCHAR prueba_32[] = "* 2 linea se dibuja de izquierda a derecha (->).";
      TextOut(hdc, width/5, (3 * height)/12 + 120, titulo3, strlen(titulo3));
      TextOut(hdc, width/5, (3 * height)/12 + 140, prueba_31, strlen(prueba_31));
      TextOut(hdc, width/5, (3 * height)/12 + 160, prueba_32, strlen(prueba_32));
      dibujarLinea(hdc, x_ini + 20, y_ini + 220, x_ini, y_ini + 220);
      dibujarLinea(hdc, x_ini + 20, y_ini + 220, x_ini + 40, y_ini + 220);
      EndPaint(hwnd, &ps);
      break;
    case WM_DESTROY:
