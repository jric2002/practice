...
// Estructura para almacenar la ruta, las distancias y la distancia total
struct Ruta {
  vector<wstring> ruta;
...
Ruta encontrarRutaMasCorta(const wstring& origen, const wstring& destino) {
  ...
  vector<wstring> ruta;
  vector<int> distancias_entre_ciudades;
  ...
  wstring ciudad_actual = destino;
  while (ciudad_actual != L"" && ruta_previa[ciudad_actual] != L"") {
    wstring ciudad_previa = ruta_previa[ciudad_actual];
    distancias_entre_ciudades.push_back(distancias[ciudad_previa][ciudad_actual]);
    ciudad_actual = ciudad_previa;
  }
  reverse(distancias_entre_ciudades.begin(), distancias_entre_ciudades.end());

  // Retornar la ruta y la distancia total
  return {ruta, distancias_entre_ciudades, distancia[destino]};
}

// Función principal para llenar los ComboBoxes con nombres de ciudades
void llenarCombobox(HWND hwnd, int idCombobox) {
  HWND combobox = GetDlgItem(hwnd, idCombobox);
  SendMessage(combobox, CB_RESETCONTENT, 0, 0);
  for (auto& ciudad : distancias) {
    SendMessage(combobox, CB_ADDSTRING, 0, (LPARAM)ciudad.first.c_str());
  }
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  switch (uMsg) {
    case WM_CREATE: {
      ...
      // Llenar los ComboBoxes con nombres de ciudades
    ...
    case WM_COMMAND: {
      ...
      if (LOWORD(wParam) == 3) { // Botón "Calcular" presionado
        ...
        wcout << L"Distancia: " << rutaMasCorta.distancia_total << endl;
        SetWindowText(GetDlgItem(hwnd, 4), (L"Distancia: " + to_wstring(rutaMasCorta.distancia_total) + L" Km").c_str());
        // Mostrar ruta más corta y distancias entre ciudades
        wstring rutaCortaStr = L"Ruta más corta: ";
        for (size_t i = 0; i < rutaMasCorta.ruta.size(); ++i) {
            rutaCortaStr += rutaMasCorta.ruta[i];
            if (i < rutaMasCorta.ruta.size() - 1) {
                rutaCortaStr += L" -> ";
                // Agregar cuadro de texto con distancia y flecha
                int x = 140 + i * 120; // Posición x para el cuadro de texto
                int y = 115;           // Posición y fija

                // Convertir la distancia a texto
                wstring distanciaTexto = to_wstring(rutaMasCorta.distancias[i]) + L" Km";

                // Crear el control estático con el texto de la distancia
                CreateWindow(L"STATIC", distanciaTexto.c_str(), WS_VISIBLE | WS_CHILD,
                            x - 10, y - 10, 70, 20, hwnd, nullptr, nullptr, nullptr);

                // Dibujar flecha con línea
                HPEN hPen = CreatePen(PS_SOLID, 2, RGB(0, 0, 0)); // Pen negro sólido
                HDC hdc = GetDC(hwnd);
                SelectObject(hdc, hPen);
                if ((i + 2) != rutaMasCorta.ruta.size()) {
                  MoveToEx(hdc, x + 70, y, nullptr);
                  LineTo(hdc, x + 100, y);
                  MoveToEx(hdc, x + 100, y, nullptr);
                  LineTo(hdc, x + 95, y - 5); // Primera punta de flecha
                  MoveToEx(hdc, x + 100, y, nullptr);
                  LineTo(hdc, x + 95, y + 5); // Segunda punta de flecha
                }
                ReleaseDC(hwnd, hdc);
                DeleteObject(hPen);
            }
        }
        // Mostrar la ruta corta en el control estático
        rutaCortaStr = rutaCortaStr.substr(0, rutaCortaStr.size() - 4); // Eliminar el último " -> "
        wcout << L"Ruta más corta: " << rutaCortaStr << endl;
    ...
    case WM_SIZE: {
      ...
      MoveWindow(GetDlgItem(hwnd, 5), 10, 130, 500, 20, TRUE);
      MoveWindow(GetDlgItem(hwnd, 6), 10, 180, 150, 20, TRUE);
      break;
    }
    case WM_CLOSE: {
      DestroyWindow(hwnd);
...
// Función principal de la aplicación WinAPI
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
  // Crear ventana principal
  WNDCLASS wc = {};
  wc.lpfnWndProc = WindowProc;
  wc.hInstance = hInstance;
  wc.lpszClassName = L"RutasCiudadesApp";
  RegisterClass(&wc);
  HWND hwnd = CreateWindowEx(
    0,                              // Optional window styles
    L"RutasCiudadesApp",            // Window class
    L"Rutas entre Ciudades",        // Window title
    WS_OVERLAPPEDWINDOW,            // Window style
    CW_USEDEFAULT, CW_USEDEFAULT, 800, 400, // Tamaño y posición de la ventana
    NULL,       // Parent window
    NULL,       // Menu
